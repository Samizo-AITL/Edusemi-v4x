...
| NCD-PH | フォトリソグラフィ（マスク露光）<br>Photolithography (Mask Exposure) | CD | Nchチャネル領域の定義<br>Define Nch channel region | - | - | - | NWL |
| NCD-ION | イオン注入（チャネルドープ）<br>Ion Implantation (Channel Doping) | CD | NMOSのしきい値調整（P-type注入）<br>Adjust NMOS Vth (P-type) | Boron, 50 keV, 1E13 cm⁻² | - | - | - |
| PCD-PH | フォトリソグラフィ（マスク露光）<br>Photolithography (Mask Exposure) | CD | Pchチャネル領域の定義<br>Define Pch channel region | - | - | - | PWL |
| PCD-ION | イオン注入（チャネルドープ）<br>Ion Implantation (Channel Doping) | CD | PMOSのしきい値調整（N-type注入）<br>Adjust PMOS Vth (N-type) | BF₂, 30 keV, 1E13 cm⁻² | - | - | - |
