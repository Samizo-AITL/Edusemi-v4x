...
flowchart LR
    %% Moore FSM構造（左）
    MooreInput["Moore FSM<br>Input"]
    MooreNSL["Next-State Logic"]
    MooreREG["State Register"]
    MooreOUT["Output"]
    ...
    %% Mealy FSM構造（右）
    MealyInput["Mealy FSM<br>Input"]
    MealyNSL["Next-State Logic"]
    MealyREG["State Register"]
    MealyOUTLOGIC["Output Logic<br>(State + Input)"]
    MealyOUT["Output"]

    MealyInput --> MealyNSL
    MealyNSL --> MealyREG
    MealyREG --> MealyOUTLOGIC
    MealyOUTLOGIC --> MealyOUT

    %% 横並びに強制するためのダミーリンク
    MooreInput -.-> MealyInput
```
