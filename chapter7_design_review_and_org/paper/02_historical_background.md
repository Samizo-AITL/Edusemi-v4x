...
一方で、LCDドライバーの用途ごとに必要とされるプロセス世代には違いがあった。  
- **モノクロLCDドライバー**では、ロジック規模が小さく、高速性の要求も低いため、安定した**0.35µmプロセス**で十分に対応可能であった。  
- **カラーLCDドライバー（aTFTパネル向け）**では、画素数の増加に伴い制御ロジックの性能向上が強く求められた。さらに大規模なSRAMマクロを内蔵する必要があり、**より高性能な0.25µmプロセス**の採用が必須となった。  

このように、**0.35µmと0.25µmは用途で棲み分け**が行われ、特にカラーTFTドライバー市場の拡大が0.25µmの延命と普及を強く後押しすることとなった。
