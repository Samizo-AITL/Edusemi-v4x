...
## 📗 本パラメータ表の出力根拠  
**Basis of Parameter Estimations**

以下のパラメータ表における各数値は、次の3つの方法に基づいて出力または補完された推定値です：

1. **🧾 公式資料・学会発表に基づく実測・公表値**  
   - 各ノードに対して、IEDM、VLSI Symposia、ISSCCなどで公表されたプロセス発表資料、ならびにTSMC, Samsung, Intel, IMEC等の発表スライドから直接引用または再構成。
...
3. **⚙️ 微細化則・物理整合性に基づく補完**  
   - 未公表の項目や不明瞭なパラメータ（例： $T_{\mathrm{ox}}$ , $V_{\mathrm{th}}$ ）については、以下のような物理法則・スケーリングトレンドに基づき補完：
     - $T_{\mathrm{ox}} \propto L_g$（短チャネル効果対策）
     - $V_{\mathrm{th}}$ は $I_{\mathrm{off}}$ 目標との整合性に基づく設計指針から設定
     - 抵抗成分（ゲート、アクティブ、コンタクト、ビア）はCMOS技術評価用テストパターンの平均値を参照

> **注記：** 本データは絶対値を保証するものではなく、**世代間の相対比較、設計指針、教育理解を目的とする**ものです。

## 🔖 参考資料 / References

- [1] **BSIM-CMG Model Manual**, UC Berkeley  
- [2] **TSMC N2 Technology Overview**, ISSCC 2024  
- [3] **Samsung SF3E Node Disclosure**, IEDM 2023  
- [4] **IMEC Advanced CMOS Roadmap**, VLSI Symposia 2022  
- [5] SemiWiki / IEEE Spectrum Articles, 2020–2024  
- [6] Internal estimation based on ITRS/IRDS trends and process benchmarking
