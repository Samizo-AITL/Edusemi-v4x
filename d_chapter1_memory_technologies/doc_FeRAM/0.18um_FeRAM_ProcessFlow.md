...
| 工程カテゴリ | 主な処理 | 目的 |
|--------------|----------|------|
| FEOL | N/Pウェル形成、チャネル注入、G1/G2酸化 | 1.8V/3.3V MOS分離 |
| Gate | ポリSiゲート、CoSi₂サリサイド | CMOS互換・低抵抗 |
| Capacitor | Pt/PZT/Ti キャパシタ + AlOx保護 | FeRAMセル形成 |
| BEOL | Wプラグ、Al配線（M1〜M3）、CMP | 配線接続 |
| Pad/Passivation | Bond Pad、SiN/SiO₂パッシベーション | 外部I/O保護 |
