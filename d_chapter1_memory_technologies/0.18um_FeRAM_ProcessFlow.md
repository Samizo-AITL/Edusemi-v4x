...
### 🔬 G1/G2酸化膜形成の正確なインテグレーション設計  
### 🔬 Correct Integration Strategy of G1/G2 Oxide Formation

0.18μm世代のCMOSプロセスでは、**1.8Vと3.3Vのデバイスを共存させるために、異なるゲート酸化膜厚（Tox）**が必要です。  
In 0.18μm CMOS processes, different gate oxide thicknesses (Tox) are required to integrate 1.8V and 3.3V transistors.

本プロセスでは、以下の2段階酸化（**G1 / G2-OX**) を用いて、両電圧域に対応します。  
This process uses a two-step oxidation scheme (G1 / G2-OX) to accommodate both voltage domains.

#### 🧪 酸化ステップの構成 / Oxidation Step Structure

| ステップ / Step | 内容 / Description | 対象領域 / Target Area | 膜厚 / Oxide Thickness |
|------------------|----------------------|---------------------------|-------------------------|
| **G1-OX** | 初期酸化（Dry OX） | 全MOS領域 / All MOS regions | 約35Å |
| **G2-OX** | 再酸化（Dry OX） | 全MOS領域 / All MOS regions | 追加35Å（合計70Å） |

- G1酸化後の段階で既に3.3V用のTox（35Å）が形成されています。  
  After G1 oxidation, the required oxide thickness for 3.3V devices (35Å) is achieved.
  
- G2では、**全領域を再度酸化**します。結果として、**1.8Vデバイスは70Å（35Å + 35Å）**となります。  
  During G2 oxidation, all regions undergo additional oxidation. As a result, **1.8V devices receive 70Å (35Å + 35Å)** oxide
