...
### 🔬 G1/G2酸化のインテグレーション戦略（選択酸化）  
### 🔬 G1/G2 Oxidation Integration Strategy (Selective Oxidation)

0.18μm CMOSプロセスにおいて、1.8V / 3.3V MOSを統合する際は、ゲート酸化膜厚の最適化が必須です。  
In 0.18μm CMOS processes, optimizing gate oxide thickness is essential when integrating 1.8V and 3.3V MOS devices.

本フローでは以下のように設計されています：  
This process adopts the following oxidation steps:

- **G1-OX**：全領域に35Åの初期酸化膜（Dry-OX）を形成  
  **G1-OX**: Initial gate oxidation (35Å) is applied uniformly across all MOS regions using dry oxidation.
  
- **G2-PH / G2-ET / G2-OX**：3.3Vデバイス領域をレジストで保護し、1.8V MOS領域のみ再酸化（追加35Å）  
  **G2-PH / G2-ET / G2-OX**: Resist masking is used to protect 3.3V regions, and a second oxidation step (+35Å) is applied only to the 1.8V device areas.

これにより、MOSごとに以下の酸化膜厚が確保されます：  
As a result, the effective gate oxide thickness for each voltage domain is as follows:

| 電圧 / Voltage | 酸化膜厚 / Oxide Thickness | 対応マスク / Applied Masking |
|----------------|-----------------------------|-------------------------------|
| 1.8V           | 35Å (G1) + 35Å (G2) = **70Å** | G1 + G2                      |
| 3.3V           | **35Å** (G1のみ / G1 only)     | G1                          |
