...
| NCD-PH        | フォトリソグラフィ         | CD          | Nchチャネル領域の定義 / Nch Channel Definition   | -                                 | **1.8V MOS**   | -         | NCD            |
| NCD-ION       | イオン注入                 | CD          | NMOSしきい値調整 / NMOS Vth Adjust               | B, 50keV, 1E13                     | **1.8V MOS**   | -         | -              |
| PCD-PH        | フォトリソグラフィ         | CD          | Pchチャネル領域の定義 / Pch Channel Definition   | -                                 | **1.8V MOS**   | -         | PCD            |
| PCD-ION       | イオン注入                 | CD          | PMOSしきい値調整 / PMOS Vth Adjust               | BF₂, 30keV, 1E13                   | **1.8V MOS**   | -         | -              |
| NCD2-PH       | フォトリソグラフィ         | CD          | Nchチャネル（3.3V）領域再定義 / 3.3V Channel     | -                                 | **3.3V MOS**   | -         | NCD            |
| NCD2-ION      | イオン注入                 | CD          | NMOSしきい値再調整 / NMOS Vth Re-adjust          | B, 50keV, 1E13                     | **3.3V MOS**   | -         | -              |
| PCD2-PH       | フォトリソグラフィ         | CD          | Pchチャネル（3.3V）領域再定義 / 3.3V Channel     | -                                 | **3.3V MOS**   | -         | PCD            |
| PCD2-ION      | イオン注入                 | CD          | PMOSしきい値再調整 / PMOS Vth Re-adjust          | BF₂, 30keV, 1E13                   | **3.3V MOS**   | -         | -              |
| G1-OX         | ゲート酸化（初期酸化）     | Gate        | 全体酸化膜（35Å）形成 / Initial Oxide Formation | Dry OX                            | 全MOS領域     | 35Å       | -              |
| G2-PH         | フォトリソグラフィ         | Gate        | 3.3V保護レジスト / Protect 3.3V Areas            | -                                 | 3.3V MOS       | -         | G2             |
| G2-ET         | 酸化膜エッチング（選択）   | Gate        | 1.8V酸化膜除去 / Remove 1.8V Oxide               | HF or SPM                         | 1.8V MOS       | 0Å        | -              |
| G2-OX         | ゲート酸化（再酸化）       | Gate        | 1.8V酸化膜再形成（+35Å）/ Re-oxidation for 1.8V | Dry OX                            | 1.8V MOS       | 35Å       | -              |
| PLY-DP        | ポリSi堆積                 | Gate        | ゲート電極形成 / Poly-Si Gate Deposition         | LPCVD                             | 共通           | 1500Å     | -              |
| PLY-PH        | フォトリソグラフィ         | Gate        | ポリゲートパターン定義 / Gate Patterning         | KrF, CD = 0.18μm                  | 共通           | 0.18μm    | PLY            |
| PLY-ET        | ポリSiエッチング            | Gate        | ゲート構造定義 / Poly Etching                    | RIE                               | 共通           | 0.18μm    | -              |
| **NLL-PH**  | Lithography    | NMOS          | 1.8V        | LDDパターン形成（ロジック用） / Logic LDD pattern | KrF             | NLL           |
| **NLL-ION** | Ion Implant    | NMOS          | 1.8V        | 浅拡散形成（ロジック） / Shallow extension for logic | As, 30keV, 1E13 | -             |
| **PLL-PH**  | Lithography    | PMOS          | 1.8V        | LDDパターン形成（ロジック用） / Logic LDD pattern | KrF             | PLL           |
| **PLL-ION** | Ion Implant    | PMOS          | 1.8V        | 浅拡散形成（ロジック） / Shallow extension for logic | BF₂, 30keV, 1E13| -             |
| **NLM-PH**  | Lithography    | NMOS          | 3.3V        | LDDパターン形成（メモリ用） / Memory LDD pattern | KrF, CD = 0.22μm| NLM           |
| **NLM-ION** | Ion Implant    | NMOS          | 3.3V        | 浅拡散形成（メモリ） / Shallow extension for memory | As, 30keV, 1E13 | -             |
| **PLM-PH**  | Lithography    | PMOS          | 3.3V        | LDDパターン形成（メモリ用） / Memory LDD pattern | KrF, CD = 0.22μm| PLM           |
| **PLM-ION** | Ion Implant    | PMOS          | 3.3V        | 浅拡散形成（メモリ） / Shallow extension for memory | BF₂, 30keV, 1E13| -             |
| **SW-DP**   | Spacer Dep.    | Gate          | 共通        | LDD保護用スペーサ堆積 / Spacer for S/D definition | SiN, LPCVD, 800Å| -             |
| **SW-ET**   | Spacer Etch    | Gate          | 共通        | スペーサアニソトロピックエッチング / Spacer RIE patterning | RIE             | -             |
| **NLL2-PH** | Lithography    | NMOS          | 1.8V        | S/D深拡散パターン（ロジック） / Logic S/D pattern | KrF, CD = 0.18μm| NLL2          |
| **NLL2-ION**| Ion Implant    | NMOS          | 1.8V        | 深拡散（ロジック） / Deep S/D for logic | As, 40keV, 1E13 | -             |
| **PLL2-PH** | Lithography    | PMOS          | 1.8V        | S/D深拡散パターン（ロジック） / Logic S/D pattern | KrF, CD = 0.18μm| PLL2          |
| **PLL2-ION**| Ion Implant    | PMOS          | 1.8V        | 深拡散（ロジック） / Deep S/D for logic | BF₂, 40keV, 1E13| -             |
| **NLM2-PH** | Lithography    | NMOS          | 3.3V        | S/D深拡散パターン（メモリ） / Memory S/D pattern | KrF, CD = 0.22μm| NLM2          |
| **NLM2-ION**| Ion Implant    | NMOS          | 3.3V        | 深拡散（メモリ） / Deep S/D for memory | As, 40keV, 1E13 | -             |
| **PLM2-PH** | Lithography    | PMOS          | 3.3V        | S/D深拡散パターン（メモリ） / Memory S/D pattern | KrF, CD = 0.22μm| PLM2          |
| **PLM2-ION**| Ion Implant    | PMOS          | 3.3V        | 深拡散（メモリ） / Deep S/D for memory | BF₂, 40keV, 1E13| -             |
